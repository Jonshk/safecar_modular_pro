...
def list_reviews(skip: int = 0, limit: int = 20):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT * FROM reviews WHERE is_approved = true ORDER BY created_at DESC LIMIT %s OFFSET %s",
        (limit, skip)
    )
...
def review_stats():
    ...
    row = dict(c.fetchone())
    c.close(); conn.close()
    row['avg_rating'] = round(float(row['avg_rating'] or 0), 1)
    return row
...
def pending_reviews():
    conn = get_connection()
    c = conn.cursor()
    c.execute("SELECT * FROM reviews WHERE is_approved = false ORDER BY created_at DESC")
    rows = [dict(r) for r in c.fetchall()]
    c.close(); conn.close()
    return rows
