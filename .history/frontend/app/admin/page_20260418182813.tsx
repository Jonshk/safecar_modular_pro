  ...
  return (
    <div className="adminLogin">
      <div className="adminLoginCard">
        ...
        <p style={{marginTop:24,fontSize:".78rem",color:"rgba(255,255,255,.2)",textAlign:"center"}}>
          Default: admin / safecar2024
        </p>
      </div>
    </div>
  );
    ...
    <div className="adminSection">
      ...
      <form onSubmit={save} className="adminForm">
        ...
        <div className="adminFormGrid">
          ...
          <label className="adminLabel" style={{gridColumn:"1/-1"}}>Image URL
            <input className="ctInput" placeholder="https://images.unsplash.com/..." value={form.image_url||""} onChange={f("image_url")} />
            <span style={{fontSize:".72rem",color:"rgba(255,255,255,.25)",marginTop:4}}>Paste any direct image URL. Use Unsplash for free high-quality photos.</span>
          </label>
  ...
  return (
    <div className="adminSection">
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <select className="ctInput" value={catFilter} onChange={e=>setCatFilter(e.target.value)}
          style={{minWidth:160,height:38,cursor:"pointer",background:"rgba(255,255,255,.05)"}}>
          ...
          {filtered.map(p => (
            <div key={p.id} className="adminPartCard">
              ...
              <div className="adminPartInfo">
                ...
                <button className="adminBtnSm" style={{marginTop:10,width:"100%"}} onClick={() => setForm({...p, price: p.price?.toString(), stock: p.stock?.toString()})}>
                  Edit →
                </button>
              </div>
            </div>
      ...
      <div className="adminDetailGrid" style={{marginBottom:16}}>
        ...
        <div className="adminDetailCard">
          ...
          {selected.payment_status === "awaiting_verification" && (
            <button className="adminBtnPrimary" style={{marginTop:14,width:"100%"}} onClick={() => confirm(selected.id)}>
              ✓ Confirm payment
            </button>
          )}
        </div>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
        {statusFilters.map(s => (
          <button key={s} onClick={() => setFilter(s)}
            style={{height:32,padding:"0 14px",borderRadius:"999px",border:"1px solid",fontSize:".78rem",fontWeight:600,cursor:"pointer",
            ...
            {s!=="all" && <span style={{marginLeft:6,opacity:.6}}>
              ({orders.filter(o=>o.payment_status===s).length})
            </span>}
          </button>
        ))}
      </div>
...
// ══ TRAINING MODULES ═══════════════════════════════════════
const EMPTY_MODULE = { title:"", title_es:"", description:"", description_es:"", duration_weeks:"4", price:"", mode:"hybrid", max_students:"20", schedule:"", image_url:"", is_active:true, sort_order:"0" };
...
function ModulesSection() {
  ...
  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const payload = { ...form, price: parseFloat(form.price), duration_weeks: parseInt(form.duration_weeks), max_students: parseInt(form.max_students), sort_order: parseInt(form.sort_order||"0") };
    const isNew = !form.id;
    await apiFetch(
  ...
  if (form !== null) return (
    <div className="adminSection">
      ...
      <form onSubmit={save} className="adminForm">
        <div className="adminFormGrid">
          <label className="adminLabel">Title (English) *<input className="ctInput" required value={form.title||""} onChange={f("title")} /></label>
          <label className="adminLabel">Title (Español)<input className="ctInput" value={form.title_es||""} onChange={f("title_es")} /></label>
          <label className="adminLabel">Price (USD) *<input className="ctInput" type="number" step=".01" required value={form.price||""} onChange={f("price")} /></label>
          <label className="adminLabel">Duration (weeks)
          ...
          <label className="adminLabel">Mode
            <select className="ctInput" value={form.mode||"hybrid"} onChange={f("mode")} style={{background:"rgba(255,255,255,.05)",cursor:"pointer"}}>
              <option value="hybrid">Hybrid</option>
              <option value="online">Online</option>
              <option value="presential">In-person</option>
            </select>
          </label>
          <label className="adminLabel">Max students<input className="ctInput" type="number" value={form.max_students||20} onChange={f("max_students")} /></label>
          <label className="adminLabel" style={{gridColumn:"1/-1"}}>Schedule<input className="ctInput" placeholder="Saturdays 9AM–1PM" value={form.schedule||""} onChange={f("schedule")} /></label>
          <label className="adminLabel" style={{gridColumn:"1/-1"}}>Description (EN)<textarea className="ctInput ctTextarea" rows={3} value={form.description||""} onChange={f("description")} /></label>
          <label className="adminLabel" style={{gridColumn:"1/-1"}}>Description (ES)<textarea className="ctInput ctTextarea" rows={3} value={form.description_es||""} onChange={f("description_es")} /></label>
          <label className="adminLabel" style={{gridColumn:"1/-1"}}>Image URL<input className="ctInput" value={form.image_url||""} onChange={f("image_url")} /></label>
        </div>
        <div className="adminFormActions">
          <button type="submit" className="cartCheckoutBtn" disabled={saving} style={{width:"auto",padding:"0 32px"}}>{saving?"Saving...":form.id?"Save changes":"Create module"}</button>
          <button type="button" className="adminBtnGhost" onClick={() => setForm(null)}>Cancel</button>
        </div>
      </form>
    </div>
  ...
  return (
    <div className="adminSection">
      ...
      {loading ? <p className="adminLoading">Loading...</p> : (
        <div className="adminTable">
          <table>
            <thead><tr><th>Title</th><th>Mode</th><th>Price</th><th>Duration</th><th>Enrolled / Max</th><th></th></tr></thead>
            <tbody>
              {modules.map(m => (
                <tr key={m.id}>
                  <td><strong>{m.title}</strong><div style={{fontSize:".75rem",color:"rgba(255,255,255,.35)"}}>{m.title_es}</div></td>
                  <td style={{textTransform:"capitalize"}}>{m.mode}</td>
                  <td><strong>${m.price?.toFixed(2)}</strong></td>
                  <td>{m.duration_weeks>0?`${m.duration_weeks} wks`:"Open"}</td>
                  <td>
                    <span style={{color: m.enrolled_count>=m.max_students?"#ef4444":"#16a34a"}}>
                      {m.enrolled_count}
                    </span>
                    <span style={{color:"rgba(255,255,255,.3)"}}> / {m.max_students}</span>
                  </td>
                  <td><button className="adminBtnSm" onClick={() => setForm({...m, price:m.price?.toString(), duration_weeks:m.duration_weeks?.toString(), max_students:m.max_students?.toString()})}>Edit</button></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="adminSection">
      ...
      {loading ? <p className="adminLoading">Loading...</p> : (
        <div className="adminTable">
          <table>
            <thead><tr><th>Reference</th><th>Student</th><th>Module</th><th>Method</th><th>Status</th><th>Amount</th><th></th></tr></thead>
            <tbody>
              {filtered.map(e => (
                <tr key={e.id}>
                  ...
                  <td>
                    {e.payment_status==="awaiting_verification" && (
                      <button className="adminBtnSm" style={{background:"rgba(22,163,74,.15)",borderColor:"rgba(22,163,74,.3)",color:"#4ade80"}}
                        onClick={() => confirm(e.id)}>✓ Confirm</button>
                    )}
                  </td>
                </tr>
              ))}
  ...
  return (
    <div className="adminSection">
      ...
      {loading ? <p className="adminLoading">Loading...</p> : (
        <div className="adminTable">
          <table>
            <thead><tr><th>#</th><th>Name</th><th>Phone</th><th>Vehicle</th><th>Issue</th><th>Date</th><th></th></tr></thead>
            <tbody>
              {quotes.map(q => (
                <tr key={q.id}>
