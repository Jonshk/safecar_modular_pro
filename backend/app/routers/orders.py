...
def create_order(data: OrderCreate):
    ...
    for item in data.items:
        ...
        total += subtotal
        items_data.append({
            "part_id": item.part_id, "part_name": part["name"],
            "quantity": item.quantity, "unit_price": part["price"], "subtotal": subtotal
        })
...
def confirm_manual(order_id: int, data: TransferConfirmIn):
    ...
    if not c.fetchone():
        c.close(); conn.close()
        raise HTTPException(404, "Order not found")
    c.execute("UPDATE orders SET payment_status='paid', confirmation_note=%s WHERE id=%s",
